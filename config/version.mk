...
# Set all versions
HAVOC_VERSION = 3.4
HAVOC_BUILD_DATE := $(shell date -u +%d-%m-%Y-%m%s)
HAVOC_BUILD_VERSION := Havoc-OS-v$(HAVOC_VERSION)-$(shell date -u +%Y%m%d)-$(HAVOC_BUILD)-$(HAVOC_BUILD_TYPE)
HAVOC_FINGERPRINT := Havoc-OS/v$(HAVOC_VERSION)/$(PLATFORM_VERSION)/$(TARGET_PRODUCT_SHORT)/$(shell date -u +%Y%m%d-%m%s)/$(shell date -u +%H%M-%m%s)
